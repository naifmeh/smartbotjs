...
function nstep_sarsa() {
    ...
    const local_utils = require('./utils.js').algo_utils;
    ...
    async function algo(num_episode, discount_factor = 1.0, alpha = 0.5, epsilon = 0.2, n_param=10, offline=true) {
        ...
        if(!offline)
            ...
            Q = local_utils.read_qvalues('plot_nstep_sarsa.js', AMOUNT_ACTIONS);
            ...
            while (true){
                ...
                if(t < n_steps) {
                    ...
                    if(done) {
                        ...
                        var next_action = math_utils.weightedRandomItem(data.actions_index, next_action_probs);
                ...
                if(pi >= 0) {
                    let returns = 0.
                    for(let x=pi+1; x<Math.min(pi+n_param, n_steps)+1;x++) {
                        returns += Math.pow(discount_factor, x-pi-1) * rewards[x];
                    }

                    if((pi+n_param) < n_steps) {
                        returns += (discount_factor**n_param) * Q[states[pi+n_param]][actions[pi+n_param]]
                    }
...
(async() => {
    ...
    let result = await sars.execute_algo(80, 1.0, 0.5, 0.1,10, false);
